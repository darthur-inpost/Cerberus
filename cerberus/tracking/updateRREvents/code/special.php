...
// The date stored in the shared hosting database is set to be WPT (Western
// Pacific Time) and not what we want. Remove eight hours from the time to
// search for.
$date = strtotime($lastLoggedDT);
if ($date === false)
{
}
else
{
    $lastLoggedDT = date("Y-m-d H:i:s", mktime(
            date('H', $date) - 8, date('i', $date), date('s', $date),
...
$conn1->query($config['queries']['qGetCurrentShipments']);
